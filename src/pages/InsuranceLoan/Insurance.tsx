...
const InsuranceTable: React.FC = () => {
  ...
  const fetchLoans = async (p = page) => {
    // This function is now handled by useInsuranceCache hook
    // Keeping for compatibility but not using setLoading since it's not defined
    try {
      ...
      // Data is now handled by useInsuranceCache hook
      // This function is kept for compatibility but data comes from the hook
    } catch (err) {
      ...
      // Loading state is handled by useInsuranceCache hook
    }
  };
  ...
  const handleReject = async () => {
    if (!selectedLoan?.id || !rejectionReason.trim()) {
      toast({
        title: "Error",
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          {detailsLoading ? (
            ...
            <div className="space-y-4">
              ...
              {/* Enhanced Loan Actions - Show different buttons based on status */}
                ...
                <div className="mt-4 bg-white rounded-lg p-4 border border-gray-200">
                  ...
                  <div className="flex justify-end gap-3">
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          const res = await fetch(`${API_BASE_URL}/api/loans/insurance_loans/${selectedLoan.id}/status`, {
                            method: 'PATCH',
                            headers: { 'Content-Type': 'application/json' },
