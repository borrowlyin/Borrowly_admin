...
const BusinessTable: React.FC = () => {
  ...
  const handleManualAssign = async () => {
    ...
    try {
      const manualAssignment = {
        assignment_id: `manual_${Date.now()}`,
        loan_id: selectedLoan.id,
        loan_type: table,
        bank_id: `manual_${Date.now()}`,
        bank_name: manualBankData.bank_name,
        ifsccode: manualBankData.ifsc_code,
        branchname: manualBankData.branch,
        bank_status: "pending",
        is_manual_entry: true,
        manual_bank_name: manualBankData.bank_name,
        manual_ifsc_code: manualBankData.ifsc_code,
        manual_branch: manualBankData.branch,
        manual_account_number: manualBankData.account_number
      };

      setAssignedBanks([manualAssignment]);
  ...
  // Filter loans based on search, status, and date
  ...
  return (
    ...
    >
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          {detailsLoading ? (
            ...
            <div className="space-y-4">
              ...
              {/* Enhanced Bank Assignment Section */}
                  ...
                  try {
                    const res = await fetch(`${API_BASE_URL}/api/loan-status/${table}/${selectedLoan.id}/approve`, {
                      ...
                      body: JSON.stringify({
                        ...
                        bankName: (assignedBanks.length > 0 && assignedBanks[0].bank_name) ? assignedBanks[0].bank_name : manualBankData.bank_name,
                        bankBranch: (assignedBanks.length > 0 && assignedBanks[0].branchname) ? assignedBanks[0].branchname : manualBankData.branch,
                        bankContact: (assignedBanks.length > 0 && assignedBanks[0].ifsccode) ? assignedBanks[0].ifsccode : manualBankData.ifsc_code
                      })
                    });
      ...
      {rejectionModal.isOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold text-red-700 mb-4">Rejection Details</h3>
            <div className="space-y-3">
              <div>
                <label className="text-sm font-medium text-gray-700">Bank Name:</label>
                <p className="text-sm text-gray-900 mt-1">{rejectionModal.bankName}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-700">Rejection Reason:</label>
                <p className="text-sm text-gray-900 mt-1 p-3 bg-red-50 border border-red-200 rounded">
