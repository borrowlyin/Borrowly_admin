...
const VehicleTable: React.FC = () => {
  ...
  const {
    ...
    lastUpdated,
    refetch,
  } = useVehicleLoanCache(page, search, statusFilter);
  ...
  const updateLoanStatus = async (loanId: string, newStatus: string) => {
    try {
      ...
      if (selectedLoan?.id === loanId)
        setSelectedLoan((s: any) => (s ? { ...s, status: newStatus } : s));
  ...
  const handleDownload = async (from?: string, to?: string) => {
    ...
    try {
      ...
      const filename = `vehicle-loans-${from || "all"}-to-${
        to || "all"
      }-${timestamp}.csv`;
  ...
  const confirmDownloadFromModal = async () => {
    ...
    await handleDownload(
      modalStartDate || undefined,
      modalEndDate || undefined
    );
  };

  const filteredLoans = loans.filter((loan) => {
    const matchesSearch = loan.full_name
      ?.toLowerCase()
      .includes(search.toLowerCase());
    const matchesStatus =
      statusFilter === "all" ||
      (loan.status || "pending").toLowerCase() === statusFilter.toLowerCase();
  ...
  const handleAssign = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/loans/${table}/${selectedLoan.id}/assign`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchLoanStatuses = async (loanId, loanType) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/loans/${loanType}/${loanId}/statuses`
      );
      const data = await res.json();
    ...
    >
      <div className="">
        ...
        <p className="mb-6 text-gray-500 text-[14px]">
          Oversee vehicle loan applications on Borrowly Loan. Verify vehicle
          details and applicant <br />
          information, and manage approvals effortlessly.
        </p>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-lg w-[95%] max-w-md p-5 z-10">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Select date range to filter loans by submitted date.
            </p>
      ...
      {showDownloadModal && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-lg w-[95%] max-w-md p-5 z-10">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Choose a date range to export (optional).
            </p>
        ...
        <div className="max-w-7xl mx-auto px-6 py-10 space-y-10">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
              <div>
                <h2 className="text-xl font-semibold">
                  {selectedLoan.fullname || selectedLoan.full_name}
                </h2>
                <p className="text-sm opacity-90">
                  {selectedLoan.emailaddress || selectedLoan.email_address}
                </p>
              </div>
                    ...
                    <PopoverContent className="w-64 p-2 max-h-60 overflow-y-auto">
                      {(() => {
                        const availableBanks = banks.filter(
                          (bank) =>
                            !assignedBanks.some(
                              (assigned) => assigned.bank_id === bank.bank_id
                            )
                        );
                        ...
                        return availableBanks.length > 0 ? (
                          availableBanks.map((bank) => {
                            ...
                            const bankName =
                              bank.bankname || bank.bank_name || bank.name;
            ...
            assignedBanks.length > 0 && (
              <div className="mt-4 bg-green-50 rounded-lg p-4 border border-green-200">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-2 h-2 bg-green-600 rounded-full"></div>
                  <h5 className="text-lg font-semibold text-green-800">
                    Assigned Banks ({assignedBanks.length})
                  </h5>
                </div>
                <div className="grid gap-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <section className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              ...
              {personalDetailsKeys
                .map((k) => k)
                .filter((k) => selectedLoan[k] !== undefined).length === 0 ? (
            ...
            <section className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              ...
              {Object.keys(selectedLoan)
                ...
                .map((key) => [key, selectedLoan[key]]).length === 0 ? (
                <p className="text-gray-500 text-sm">
          ...
          <section className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <div className="flex justify-between items-center border-b pb-3 mb-4">
              ...
              } / ${documentsKeys.length} Uploaded`}</span>
            </div>
                  ...
                  >
                    <FileText
                      className={`w-8 h-8 mb-3 ${
                        isUploaded ? "text-blue-700" : "text-gray-400"
                      }`}
                    />
                    ...
                    {isUploaded ? (
                      <button
                        onClick={async () => {
                          ...
                          if (signed) {
                            try {
                              const testResponse = await fetch(signed, {
                                method: "HEAD",
                              });
                              if (testResponse.ok) {
                                setDocumentModal({
            ...
            <div className="mt-4 bg-white rounded-lg p-4 border border-gray-200">
              ...
              <div className="flex justify-end gap-3">
                <Button
                  ...
                  onClick={async () => {
                    try {
                      const res = await fetch(`${API_BASE_URL}/api/loans/vehicle_loans/${selectedLoan.id}/status`, {
                        method: 'PATCH',
                        headers: { 'Content-Type': 'application/json' },
                ...
                filteredLoans.map((loan) => (
                  ...
                  >
                    ...
                    <td className="p-3 border-b text-blue-600 hover:text-blue-800 cursor-pointer font-medium">
                      <span onClick={() => fetchLoanDetails(loan.id)}>
                        View
                      </span>
                    </td>
                  </tr>
          ...
          <div className="bg-white p-6 rounded-lg max-w-4xl w-full relative">
            ...
            {docLoadError && (
              <div className="mt-4 p-4 bg-yellow-100 border border-yellow-300 text-yellow-800 rounded-md">
                Failed to load the document. It may be restricted or
                unavailable.
              </div>
            )}
          </div>
