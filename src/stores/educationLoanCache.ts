...
class EducationLoanCacheStore {
  ...
  async fetchData(page: number, search: string, statusFilter: string) {
    ...
    try {
      ...
      params.append('table', 'education_loans');
      ...
      const mapped = items.map((loan: any) => ({
        ...
        email_address: loan.email ?? loan.emailaddress ?? loan.email_address ?? '',
      ...
      console.error('Education loan fetch error:', error);
      throw error;
    }
  }
...
export const educationLoanCache = new EducationLoanCacheStore();