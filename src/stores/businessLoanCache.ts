...
class BusinessLoanCacheStore {
  ...
  async fetchData(page: number, search: string, statusFilter: string) {
    ...
    try {
      ...
      params.append('table', 'business_loans');
      ...
      console.error('Business loan fetch error:', error);
      throw error;
    }
  }
...
export const businessLoanCache = new BusinessLoanCacheStore();